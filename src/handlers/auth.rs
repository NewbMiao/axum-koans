...
use crate::{database::profile::Profile, errors::ServerError, extensions::google_auth::GoogleAuth};
...
pub async fn auth_callback_handler(
    ...
    let res = profile.save(db_pool).await?;
    Ok(Json(json!({ "google": res })))
}
